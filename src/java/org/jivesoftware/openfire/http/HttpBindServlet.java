...
public class HttpBindServlet extends HttpServlet {
            ...
            throws IOException
    {
        ...
        synchronized (session) {
            try {
                respond(session, request, response, session.getResponse((Long) request.getAttribute("request")),
                        request.getMethod());
            }
            ...
            throws IOException
    {
        ...
        synchronized (session) {
            ...
            if ("terminate".equals(type)) {
                ...
                try {
                    respond(session, request, response, session.getResponse(connection.getRequestId()),
                            request.getMethod());
                }
                catch (HttpBindException e) {
                    sendError(request, response, e.getBindingError(), session);
